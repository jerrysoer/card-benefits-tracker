...
export default function ProgramBlock({ program, balance, valuation, onUpdate }: ProgramBlockProps) {
  ...
  if (editing) {
    return (
      <div
        ...
        style={{ borderColor: program.color, backgroundColor: "var(--bg-card)" }}
      >
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 w-full rounded border border-border bg-bg-elevated px-2 py-1 font-mono-data text-sm text-text-primary focus:border-accent focus:outline-none"
            autoFocus
          />
        </div>
        <div>
        ...
        <div className="flex gap-2">
          <button
            onClick={handleSave}
            className="flex-1 rounded bg-accent px-2 py-1 text-xs font-bold text-white transition-opacity hover:opacity-80"
          >
            Save
          </button>
          <button
            onClick={() => setEditing(false)}
            className="flex-1 rounded border border-border px-2 py-1 text-xs text-text-muted transition-colors hover:text-text-primary"
          >
            Cancel
          </button>
        </div>
  ...
  return (
    <button
      ...
      className={`flex w-36 shrink-0 flex-col rounded-lg border-2 p-4 text-left transition-colors hover:bg-bg-elevated ${
        isDimmed ? "opacity-40" : ""
      }`}
      style={{ borderColor: program.color, backgroundColor: "var(--bg-card)" }}
    >
      ...
      <span className="font-mono-data text-sm font-bold text-gold">
